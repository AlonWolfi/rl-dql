...
def dqn_learing(
    ...
    for t in count():
        ...
        idx_stored = replay_buffer.store_frame(last_obs)
        recent_obs = replay_buffer.encode_recent_observation()

        action = select_epilson_greedy_action(Q, recent_obs, t)
        last_obs, reward, done, _ = env.step(action)
        replay_buffer.store_effect(idx_stored, action, reward, done)
        ...
        if (t > learning_starts and
            ...
            obs_batch = Variable(torch.Tensor(obs_batch).type(dtype)) / 255.0
            rew_batch = Variable(torch.Tensor(rew_batch).type(dtype))
            next_obs_batch = Variable(torch.Tensor(next_obs_batch).type(dtype)) / 255.0


            next_q = torch.zeros(len(done_mask)).type(dtype)
            next_q[done_mask != 1] = torch.max(target_Q(next_obs_batch[done_mask != 1]), dim=1).values
            cur_q = Q(obs_batch)[list(range(batch_size)), act_batch]
            d_error = rew_batch + gamma*next_q - cur_q
            d_error = -d_error.clamp(-1, 1)

            optimizer.zero_grad()
            cur_q.backward(d_error)
            optimizer.step()
